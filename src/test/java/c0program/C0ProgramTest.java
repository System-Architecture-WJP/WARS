...
import com.wars.c0program.Disk;
...
public class C0ProgramTest {
    ...
    void test_single_int_min_constant_program(){
        // Setup
        // Comment: -2147483648 cannot generated
        String code = "int a; int main(){a = -2147483647; return 1}~";
        C0Program pr = new C0Program(code);
        byte [] byteCode = CodeTranslation.MIPSTranslationByteArray(pr.mipsCode);
        config.setByteArray(byteCode, 0);
    ...
    void test_disk_copyms(){
        String code = "uint val; void writems(uint x, uint a){gpr(1) = x; gpr(2) = a {1}; asm( sw 1 2 0 )}; void copyms(uint a, uint b, uint L){gpr(1) = a; gpr(2) = b {1}; gpr(3) = L {1, 2}; asm( blez 3 7 ); asm( lw 4 1 0 ); asm( sw 4 2 0 ); asm( addi 1 1 4 ); asm( addi 2 2 4 ); asm( addi 3 3 -1 ); asm( blez 0 -6 )}; int main(){uint a; int n; n = 2; a = 4100u; val = 22u; while n>0 {writems(val, a); val=val+1u; a=a+4u; n=n-1}; copyms(4100u, 4200u, 5u); return 1}~";
        C0Program pr = new C0Program(code);
        byte [] byteCode = CodeTranslation.MIPSTranslationByteArray(pr.mipsCode);
        config.setByteArray(byteCode, 0);
        Simulator.simulate(config);
